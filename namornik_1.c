...
#define NUM_OF_COMP 2000000     // pocet kdy namornik spadne do vody, nebo dojde do cile

#define X           7           // velikost tabulky v x
...
#define START   (Y * X) - 4     // startovaci pozice namornika
...
void grid_init(void) {
    ...
    for (int i = 1; i < SIZE; i += X) {
        grid[i] = WATER;
    }

    for (int i = 5; i < SIZE; i += X) {
        grid[i] = WATER;
    }


    for (int i = 6; i < SIZE; i += X) {
        grid[i] = WATER;
    }
